...
- zero-allocation / low-GC 最佳化
- 完整 structured logging API
- log rotation、壓縮、遠端聚合、全文索引

但目前設計已預留擴充點：

- `LogEntry.Attrs` 可承接 JSON structured fields
- `LogHandler` 可擴充成 remote sink、metrics、alerting
- `LogStore` 可直接新增 `sqlite`、`postgres`、`s3` 等 backend
